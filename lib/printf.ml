...
let sprintf =
  let rec go : type a b. Buffer.t -> (a, b) fmt -> (unit -> a) -> b =
    fun buf fmt k -> match fmt with
    (* b = a *)
    | Lit s -> Buffer.add_string buf s; k ()
    (* (int/string/bool -> a) = b *)
    | FInt -> (fun n -> Buffer.add_string buf (Int.to_string n); k ())
    | FString -> (fun s -> Buffer.add_string buf s; k ())
    | FBool -> (fun b -> Buffer.add_string buf (Bool.to_string b); k ())
    (* bc : *)
    | Cat (bc, ab) ->
