...
let test_int () =
  Alcotest.(check int) "literal" 42 (eval (Int 42))

let test_add () =
  Alcotest.(check int) "add" 3 (eval (Add (Int 1, Int 2)))
  ...
  if depth <= 0 then
    ...
    oneof [
      ...
      map2 (fun a b -> Add (a, b))
        (gen_int_expr (depth - 1)) (gen_int_expr (depth - 1));
